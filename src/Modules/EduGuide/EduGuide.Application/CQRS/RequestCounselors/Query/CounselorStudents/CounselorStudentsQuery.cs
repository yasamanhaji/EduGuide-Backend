...
{
    public class CounselorStudentsQuery 
        : IRequest<Result<ItemListDTO<RequestCounselorGetListDTO>>>
    {
        ...
        public Expression<Func<RequestCounselor, bool>> GetFilter()
        {
            ...
            if (Status != null)
                ...
                filter.And(x => x.status == RequestStatus.Settlement || x.status == RequestStatus.Ended);

            return filter;
        }
    }

    public class CounselorStudentsQueryHandler(IEduGuideUnitOfWork uow, IJwtManager jwtManager, IMinIoService minIoService)
        : IRequestHandler<CounselorStudentsQuery, Result<ItemListDTO<RequestCounselorGetListDTO>>>
    {
        public async Task<Result<ItemListDTO<RequestCounselorGetListDTO>>> Handle(CounselorStudentsQuery request, CancellationToken cancellationToken)
        {
            var sort = "id desc";
